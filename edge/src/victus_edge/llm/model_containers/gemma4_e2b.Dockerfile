# victus_edge Gemma 4 E2B-it runtime image for Jetson Orin Nano Super.
#
# Model: lmstudio-community/gemma-4-E2B-it-GGUF  (Apache 2.0)
#   Q4_K_M:  ~1.7 GB; total params 5B, active per-token 2B
#   mmproj:  BF16 vision projector (matches E4B sibling layout)
#   Memory budget: comfortably fits the 8 GB Jetson with -c 8192;
#                  ~3 GB headroom over the E4B build for larger context
#                  windows or running alongside the webcam pipeline.
#
# This is the smaller multimodal sibling of gemma4.Dockerfile's E4B-it
# checkpoint. Pick this image when you want lower VRAM footprint and
# faster TTFT than E4B at the cost of some quality on harder prompts.
#
...
#   ~/models/gemma4/mmproj-gemma-4-E2B-it-BF16.gguf
#
# Build (run from project root — needs shared/ in build context):
#   docker build -f edge/src/victus_edge/llm/model_containers/gemma4_e2b.Dockerfile \
#                -t victus_edge/gemma4-e2b:latest .
#
# Run (production — Foundry comms loop):
#   docker run --runtime nvidia --rm --network host \
#     -v ~/models:/opt/models:ro --env-file .env \
#     victus_edge/gemma4-e2b:latest
#
# Run (--test — VLM describes webcam frames, no Foundry):
#   docker run --runtime nvidia --rm --network host \
#     -v ~/models:/opt/models:ro \
#     --device /dev/video0 \
#     victus_edge/gemma4-e2b:latest --webcam 0 --test
...
ENV PYTHONPATH=/opt/victus_edge/src \
    ...
    VICTUS_LLM_MODEL_PATH=/opt/models/gemma4/gemma-4-E2B-it-Q4_K_M.gguf \
    VICTUS_LLM_MMPROJ_PATH=/opt/models/gemma4/mmproj-gemma-4-E2B-it-BF16.gguf
