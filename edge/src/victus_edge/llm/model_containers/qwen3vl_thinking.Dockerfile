# victus_edge Qwen3-VL 2B Thinking runtime image for Jetson Orin Nano Super.
#
# Model: Qwen/Qwen3-VL-2B-Thinking-GGUF (Apache 2.0)
#   Q4_K_M:  ~1.1 GB on disk, ~1.5 GB GPU resident (matches Instruct sibling)
#   mmproj:  ~782 MB (vision projector)
#
# This is the reasoning ("Thinking") sibling of qwen3vl.Dockerfile's Instruct
# checkpoint. It emits a <think>...</think> reasoning trace before the final
# answer; on llama-server the trace surfaces in `reasoning_content` until the
# final-answer marker (handled by http_client.py's content fallback). Expect
# higher first-token latency than the Instruct variant — pick this image when
# you want chain-of-thought visual reasoning, not low-latency captioning.
#
...
#   ~/models/qwen3vl/Qwen3VL-2B-Thinking-Q4_K_M.gguf
#   ~/models/qwen3vl/mmproj-Qwen3VL-2B-Thinking-F16.gguf
#
# Build (run from project root — needs shared/ in build context):
#   docker build -f edge/src/victus_edge/llm/model_containers/qwen3vl_thinking.Dockerfile \
#                -t victus_edge/qwen3vl-thinking:latest .
#
# Run (production — Foundry comms loop):
#   docker run --runtime nvidia --rm --network host \
#     -v ~/models:/opt/models:ro --env-file .env \
#     victus_edge/qwen3vl-thinking:latest
#
# Run (--test — VLM describes webcam frames, no Foundry):
#   docker run --runtime nvidia --rm --network host \
#     -v ~/models:/opt/models:ro \
#     --device /dev/video0 \
#     victus_edge/qwen3vl-thinking:latest --webcam 0 --test
...
ENV PYTHONPATH=/opt/victus_edge/src \
    ...
    VICTUS_LLM_MODEL_PATH=/opt/models/qwen3vl/Qwen3VL-2B-Thinking-Q4_K_M.gguf \
    VICTUS_LLM_MMPROJ_PATH=/opt/models/qwen3vl/mmproj-Qwen3VL-2B-Thinking-F16.gguf
