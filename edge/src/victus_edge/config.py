...
Foundry URLs as optional — used by main.py's `--test` mode, where the
...
def load(overrides: dict[str, Any] | None = None) -> Config:
    ...
    foundry_listener_url = pick("foundry_listener_url", "FOUNDRY_LISTENER_URL") or (
        "" if test_mode else _required("FOUNDRY_LISTENER_URL")
    )
    foundry_functions_url = pick("foundry_functions_url", "FOUNDRY_FUNCTIONS_URL") or (
        "" if test_mode else _required("FOUNDRY_FUNCTIONS_URL")
    )
    ...
    return Config(
        drone_id=drone_id,
        foundry_listener_url=foundry_listener_url,
        foundry_functions_url=foundry_functions_url,
